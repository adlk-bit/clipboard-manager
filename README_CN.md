...
- 仅打包 sql.js 运行时入口以及中英文 Electron 语言资源，Windows 安装包从 99.35 MB 降至 87.66 MB。
- 移除应用归档中未使用的 sql.js 调试版、浏览器版、Worker 文件及渲染端依赖，进一步缩小体积。
- 剪贴板图片每次捕获只进行一次 PNG 编码，并复用结果完成哈希和落盘，降低大图片带来的瞬时内存峰值。
- 历史图片缩略图改为延迟、异步解码，长历史列表占用更低、滚动更流畅。
