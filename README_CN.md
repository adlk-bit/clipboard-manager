...
| 🔍 **实时搜索** | 按文字、链接、图片筛选；空格分隔的多个关键词同时匹配内容、目录或标签，支持按原字符查找 `%` / `_` |
...
| ☑️ **批量管理** | 顶部进入多选，可批量删除或将多条文字合并复制；支持换行、空行、逗号、制表符和倒序预览 |
...
- 查找与复制：新增文字/链接/图片筛选、多关键词搜索，修复快速输入和切换页面时旧搜索覆盖新结果的问题。
- 键盘操作：`Ctrl+F` 定位搜索框，`↑/↓` 在搜索时选择结果，`Enter` 复制并收起窗口，`Ctrl+1`–`Ctrl+9` 复制当前列表前九条；`Space` 编辑选中的文字，`Esc` 退出多选、清除搜索或收起窗口。输入法选字和弹窗编辑不触发后台复制。
- 文本处理与合并：先预览再复制，支持撤销；修改后的文字上限为 10,000 字，超限会提示且不会静默截断。JSON 工具保留大整数、小数精度、重复键和原有转义。
- 性能与隐私：减少键盘导航和多选时的卡片刷新；只在设置页按需刷新图片占用统计；复制提示不再展示敏感原文。

- Android 配套端同步为 1.2.0（版本代码 7），既有手机功能与配对方式保持兼容。

下载文件与验证边界见 [v1.2.0 发布说明](docs/releases/v1.2.0.md)，改进依据与后续功能优先级见 [效率改进说明](docs/efficiency-improvements.md)。
...
- 原生风格标题栏新增可持久化的窗口置顶按钮，提供明确的激活状态，并对 Windows 状态切换进行确认。
- 扩展隔离 Electron 运行验证，覆盖脱敏/显示、元数据搜索、隐私设置和置顶状态跨重启持久化，不触碰真实用户数据库。
...
本项目代码遵循 [MIT License](LICENSE) 协议发布。在保留原始版权声明和许可声明的前提下，您可以使用、复制、修改、合并、发布、分发、再许可和/或销售本软件的副本。完整许可条款请参阅 [LICENSE](LICENSE)。
