...
export default function Sidebar() {
  ...
  return (
    <aside className="app-sidebar w-[76px] shrink-0 bg-white/72 dark:bg-[#2c2c2e]/82 border-r border-white/70 dark:border-white/10 flex flex-col items-center py-4 gap-2 backdrop-blur-xl">
      {/* App icon */}
      ...
      {navItems.map((item) => (
        <button
          ...
          className={`no-drag flex flex-col items-center gap-0.5 w-[60px] py-2 rounded-[12px] transition-[background-color,color,transform] duration-150 active:scale-[0.96] ${
            currentPage === item.id
              ? 'bg-white/90 dark:bg-white/12 text-[#007aff] dark:text-[#0a84ff] shadow-[0_1px_2px_rgba(60,60,67,0.08)]'
              : 'text-[#636366] dark:text-[#98989d] hover:bg-white/65 dark:hover:bg-white/8 hover:text-[#007aff] dark:hover:text-[#0a84ff]'
          }`}
        >
